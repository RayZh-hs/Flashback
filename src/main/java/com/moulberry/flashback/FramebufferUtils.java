...
public class FramebufferUtils {
    ...
    public static void blit(RenderTarget from, RenderTarget to) {
        drawFullscreenQuad(to, ShaderManager.BLIT_SCREEN_WITH_UV, new Vector4f(1.0f, 1.0f, 1.0f, 1.0f), renderPass -> {
            renderPass.bindSampler("InSampler", from.getColorTextureView());
        });
    }

    public static void accumulate(RenderTarget to, RenderTarget accum, RenderTarget base, RenderTarget pass, float opacity) {
        drawFullscreenQuad(to, ShaderManager.BLIT_SCREEN_ACCUMULATE, new Vector4f(1.0f, 1.0f, 1.0f, opacity), renderPass -> {
            renderPass.bindSampler("AccumSampler", accum.getColorTextureView());
            renderPass.bindSampler("BaseSampler", base.getColorTextureView());
            renderPass.bindSampler("PassSampler", pass.getColorTextureView());
        });
    }

    private static void drawFullscreenQuad(RenderTarget to, RenderPipeline pipeline, Vector4f colour, Consumer<RenderPass> samplerBinder) {
        int width = to.width;
        ...
        try (MeshData meshData = builder.buildOrThrow()) {
            ...
            try (RenderPass renderPass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "flashback blit", to.getColorTextureView(), OptionalInt.empty())) {
                ...
                samplerBinder.accept(renderPass);
    ...
    public static void blitToScreenPartial(RenderTarget renderTarget, int width, int height, float x1, float y1, float x2, float y2) {
        ...
        drawPartial(renderTarget.getColorTextureView(), tempRenderTarget, width, height, x1, y1, x2, y2);
    ...
    private static void drawPartial(GpuTextureView from, RenderTarget to, int width, int height, float x1, float y1, float x2, float y2) {
        var modelViewStack = RenderSystem.getModelViewStack();
        modelViewStack.pushMatrix();
        modelViewStack.set(new Matrix4f().translation(0.0f, 0.0f, -2000.0f));
        var oldProjectionMatrix = RenderSystem.getProjectionMatrixBuffer();
